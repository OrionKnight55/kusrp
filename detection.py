...
class Yolo():
    ...
    def detect(self, frame):
        ...
        unconnected_out_layers = self.yolo.getUnconnectedOutLayers()
        
        # Check if unconnected_out_layers is an array and flatten it
        if isinstance(unconnected_out_layers, np.ndarray):
            unconnected_out_layers = unconnected_out_layers.flatten()

        # Ensure unconnected_out_layers is iterable
        output_layer = [layers_names[i - 1] for i in unconnected_out_layers]
    ...
    def process_output(self, input_img, frames):
        ...
        if len(indices) > 0 and isinstance(indices[0], list):
            indices = [i[0] for i in indices]  # Flatten the list of lists
